    ...
    <div className="app-wrapper min-vh-100">
      <nav className="navbar navbar-expand-lg navbar-dark bg-dark shadow sticky-top py-3">
        <div className="container">
          <span className="navbar-brand fw-bold fs-4">🏭 FishERP 2.0</span>
          <div className="d-flex align-items-center gap-3">
             <span className="badge bg-primary p-2 px-4 rounded-3 me-1">{roleRu(user.role)}: {user.fullName}</span>
             <button className="btn btn-danger px-4 rounded-3 fw-bold" onClick={logout}>ВЫХОД</button>
          </div>
        </div>
      </nav>
      ...
      <div className="container mt-4 pb-5">
        ...
        <div className="row mb-4">
            {user.role === 'STOREKEEPER' && getDeficitItems().length > 0 && (
                <div className="col-12 animate-in">
                    <div className="card border-0 shadow-sm rounded-4 p-4 bg-white border-start border-danger border-5">
                        <h5 className="fw-bold text-danger mb-3">⚡ Срочно к пополнению (Дефицит менее 200 шт)</h5>
                        <div className="d-flex flex-wrap gap-2">
                            {getDeficitItems().map(p => (
                                <span key={p.id} className="badge bg-danger-subtle text-danger p-2 px-3 rounded-pill" style={{fontSize: '0.9rem'}}>
                ...
                <div className="col-12 animate-in">
                    <div className="card border-0 shadow-sm rounded-4 p-3 bg-white border-start border-success border-5">
                        <div className="d-flex align-items-center text-success fw-bold">
                            <span className="fs-4 me-2">✅</span> Склад укомплектован: дефицитных позиций нет
                        </div>
                    </div>
                </div>
            )}
                    ...
                    <div className="card border-0 shadow-sm rounded-4 p-4 bg-primary text-white">
                        <div className="row align-items-center">
                            <div className="col-md-8">
                                <h5 className="fw-bold mb-1">📊 Оценка товарных запасов</h5>
                                <p className="opacity-75 mb-0">Суммарная балансовая стоимость ТМЦ на складе завода</p>
                            </div>
                            <div className="col-md-4 text-md-end">
                                <h2 className="fw-extrabold mb-0">{getTotalValue().toLocaleString()} ₽</h2>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            {user.role === 'SALES_MANAGER' && (
                <div className="col-12 animate-in">
                    <div className="card border-0 shadow-sm rounded-4 p-4 bg-white">
                        <h5 className="fw-bold mb-3">📈 Структура складских запасов</h5>
                        <div className="row">
                            {getCategoryStats().map(s => (
                                <div key={s.name} className="col-md-4 mb-3">
                                    ...
                                    <div className="text-end small fw-bold">{s.percent}%</div>
                                </div>
                            ))}
                        </div>
                    </div>
        ...
        {user.role === 'STOREKEEPER' && (
            <div className="card p-4 shadow-sm border-0 rounded-4 mb-4 bg-white">
                <h6 className="fw-bold text-muted text-uppercase mb-3">Приемка новой партии товара</h6>
                    ...
                    <div className="col-md-3">
                        <div className="d-flex align-items-center gap-2">
                            ...
                            <div className="input-group bg-light rounded-3 overflow-hidden">
                                <input className="form-control border-0 bg-transparent" placeholder="Ссылка на фото или файл..." value={newProduct.photoUrl} onChange={e=>{setNewProduct({...newProduct, photoUrl: e.target.value}); setProductError('');}} />
        ...
        <div className="card shadow-sm p-4 mb-4 border-0 rounded-4 bg-white d-flex flex-row align-items-center gap-3">
            ...
            <select className="form-select rounded-pill w-25 bg-light border-0" value={selectedCategory} onChange={e => setSelectedCategory(e.target.value)}>
            ...
            <table className="table modern-table align-middle">
                <thead>
                    <tr>
                        ...
                        <th style={{width: '150px'}}>Остаток</th>
                        {user.role !== 'STOREKEEPER' && <th style={{width: '150px'}}>Цена</th>}
                        <th className="text-end pe-4" style={{width: '120px'}}>Редакт.</th>
                    </tr>
                </thead>
                <tbody>
                    {products.map(p => (
                            ...
                            <td className="text-end pe-4">
                                {user.role === 'STOREKEEPER' ? (
                                    <div className="d-flex gap-2 justify-content-end align-items-center">
                                        <label className="btn btn-sm btn-light rounded-circle mb-0" title="Загрузить фото">
                                            📁<input type="file" hidden accept="image/*" onChange={(e) => uploadPhoto(e, p)} />
                                        </label>
                                        <button className="btn btn-sm btn-light rounded-circle" title="Ссылка на фото" onClick={()=>{const url=prompt('URL фото:', p.photoUrl); if(url) updateProduct({...p, photoUrl:url})}}>⚙️</button>
                                        <button className="btn btn-sm btn-outline-danger rounded-circle" title="Удалить" onClick={() => deleteProduct(p.id)}>🗑️</button>
                                    </div>
